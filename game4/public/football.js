...
socket.on('moving_ball',(ballz)=>{
    ...
    let b=ballz.split("/");
    if(b[6]==opponent_name || b[6]==name_of_player){
...
function move_ball(){
    ...
    if(i_am_player1){
        socket.emit('moving_ball',ball_posn_x+"/"+ball_posn_y+"/"+ball_dir_x+"/"+ball_dir_y+"/"+ball_speed+"/"+current_rotation+"/"+name_of_player);
